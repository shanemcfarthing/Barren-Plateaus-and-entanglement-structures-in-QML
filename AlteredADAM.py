...
class TrackingResult:
    def __init__(self, num_qubits, num_reps, entanglement_mode, gradients, file_dir, entropies=None):
        self.num_qubits = num_qubits    # the number of qubits in the circuit
        self.num_reps = num_reps        # the number of repetitions of the ansatz in the circuit
        self.entanglement = entanglement_mode      # the entanglement scheme used in the ansatz in the circuit
        self.gradients = gradients      # the list of gradients calculated during the optimization process
        self.params_file = file_dir     # the path to the adam_params.csv file for the circuit
        self.entropies = entropies      # the von Neumann entropies of entanglement during the optimization process


class AlteredADAM(Optimizer):
    ...
    def minimize(
        ...
        # these are used to track the gradients and loss values for the
        # different parameter configurations generated for each iteration
        ...
        # store the norm of the gradient vector, and also store the result
        # of evaluating the objective function using this set of parameters.

        gradients.append(np.linalg.norm(derivative))
        ...
        while self._t < self._maxiter:
            ...
            if self._t > 0:
                ...
                # store the values of interest using the same
                # process as before
    ...
    def optimize(
        ...
        point, value, nfev, gradients = self.minimize(objective_function, initial_point, gradient_function)
        return point, value, nfev, gradients
