...
class TrackingResult:
    def __init__(self, num_qubits, num_reps, entanglement_mode, loss, loss_differences, file_dir, entropies=None):
        ...
        self.loss = loss                # the loss values at each training iteration
        self.loss_differences = loss_differences    # the list of values for average cost - cost at each training epoch
...
class AlteredSPSA(Optimizer):
    ...
    def save_params(self, snapshot_dir: str) -> None:
        """Save the current iteration parameters to a file called ``spsa_params.csv``.
        ...
        """
        with open(os.path.join(snapshot_dir, "spsa_params.csv"), mode="a") as csv_file:
            fieldnames = ["v", "m", "t"]
    ...
    def _minimize(self, loss, initial_point):
        ...
        # variables for writing parameters to file in same format as for ADAM optimizer
        self._t = 0
